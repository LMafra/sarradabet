...
export const categoryFactory = Factory.define<CategoryAttrs>(({ sequence }) => ({
  id: sequence,
...
export const oddFactory = Factory.define<OddAttrs>(({ sequence }) => ({
  ...
  value: Number(faker.number.float({ min: 1.01, max: 10, multipleOf: 0.01 }).toFixed(2)),
  betId: undefined,
  result: null,
}));
...
export const betWithOddsFactory = Factory.define<BetWithOddsAttrs>(
  ({ sequence, associations }) => {
    const categoryId = associations.categoryId ?? faker.number.int({ min: 1, max: 1000 });
    const odds = oddFactory.buildList(2).map((o) => ({ ...o, betId: sequence, totalVotes: 0 }));
    return {
      id: sequence,
      title: faker.commerce.productName(),
...
export const adminFactory = Factory.define<{ username: string; email: string; password: string }>(() => ({
